...
import { NoDefaultStoreIdentifierError, NoDefaultStoreError, NoStoresError, BadStoreIdentifierError, CurrencyUpdateError } from '../../utils/errors'
...
export const createEnsureStore = (spreeClient: MultiCurrencySpreeClient) => (
  (request: StoreCodeRequest, _response, next) => {
    ...
    logger.info(`Multi store request. Comparing currency in request store ${request.storeConfiguration.identifier} with currency in order.`)
    ...
    spreeClient.cart.show(token)
      .then((spreeCartShowResponse) => {
        if (spreeCartShowResponse.isSuccess()) {
          ...
          spreeClient.currency.update(token, { currency: request.storeConfiguration.spreeCurrency })
