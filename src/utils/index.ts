...
const makeLogger = () => {
  ...
  return winston.createLogger({
    ...
    format: combine(timestamp(), json({ replacer, space: 2 })),
    level: 'info',
    transports: [
...
const getMediaGallery = (images: SpreeProductImage[]): ESImage[] => {
  return images.reduce((acc, _, imageIndex) => {
...
const findIncluded = (response: JsonApiResponse, objectType: string, objectId: string): JsonApiDocument | null => {
  ...
  return (
    response.included.find((includedObject) => includedObject.type === objectType && includedObject.id === objectId) ||
    null
  )
}
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    const handlePage = (page: number) => {
      if (page >= maxPages) {
        ...
        makePaginationRequest(page, perPage).then((result) => {
          if (result.isSuccess()) {
            ...
            responseResources.forEach((resource: JsonApiDocument, resourceIndex: number) => {
              try {
                ...
                logger.error([
                  ...
                  { page, per_page: perPage, resourcePageIndex: resourceIndex },
                  error
                ])
              }
            })
...
const pushElastic = (
  ...
  return pendingOperations.then(({ errors, operations, operationsCount }) => {
    ...
    const updatedOperations = [...operations, ...newOperations] as ElasticOperation[]
    if (expectedOperationsCount === maxOperationsPerBulk) {
      return flushElastic(
        elasticClient,
        Promise.resolve({
...
const flushElastic = (elasticClient, pendingOperations: ESBulkPromise): ESBulkPromise => {
  return pendingOperations.then(({ errors, operations, operationsCount }) => {
...
const pushElasticIndex = (elasticClient, pendingOperations, bulkSize, index, type, document): ESBulkPromise => {
  return pushElastic(elasticClient, pendingOperations, bulkSize, index, [
    {
      index: {
        _id: document.id,
        _index: index,
...
const pushElasticUpdate = (elasticClient, pendingOperations, bulkSize, index, type, documentPatch): ESBulkPromise => {
  return pushElastic(elasticClient, pendingOperations, bulkSize, index, [
    {
      update: {
